/**
 * safeFetch for Node.js runtime: UA, total wall-time deadline, retries.
 * Mirrors the worker version's contract so adapters port between runtimes.
 *
 * Uses Node 22+ built-in fetch (no extra deps).
 */
...
export interface SafeFetchOptions extends Omit<RequestInit, "headers" | "signal"> {
  /** Plain-object headers; merged on top of defaults. */
  headers?: Record<string, string>;
  /** Total wall-time budget across all attempts including backoff. Default 15s. */
  totalTimeoutMs?: number;
  /** Number of retries (so total attempts = retries + 1). Default 1. */
  retries?: number;
}
