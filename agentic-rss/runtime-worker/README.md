...
适用：源站有 RSS（A/B 模式），或源站没 RSS 但 HTML/API 解析够轻量（C/D 模式），且 Cloudflare 边缘 IP 段对该源站不被反爬。如果 CF 段被打，改用 `runtime-nodejs/`。
...
6. 接入消费方：把消费方源列表里那一行的 URL 改成 worker URL
...
`lib/` 和 `types.ts` 要改的话回上游 `agentic-rss/runtime-worker/` 改，那里的版本是工具维护方持有的公共代码，所有 instance 共享。
