...
def fetch_articles():
    ...
    try:
        ...
        if config["spider_settings"]["enable"]:
            ...
            logging.info(f"正在从 {json_url} 中获取，每个博客获取 {article_count} 篇文章，并发 {max_workers} (IPv4-only={force_ipv4})")
...
async def root():
    return HTMLResponse(content="<h1>Friend Circle API</h1><p>Endpoints: /all, /errors, /random</p>")
